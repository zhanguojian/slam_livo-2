...
void Preprocess::livox_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  ...
  printf("[ Preprocess ] Input point number: %zu \n", plsize);
  ...
  const double frame_start_ns =
      static_cast<double>(msg->header.stamp.sec) * 1e9 +
  ...
  if (feature_enabled)
  {
         ...
         ++i, ++iter_x, ++iter_y, ++iter_z, ++iter_i, ++iter_tag, ++iter_line, ++iter_t)
    {
      ...
      if (line < N_SCANS)
      {
        ...
        {
          is_new =
              (std::fabs(p.x - last_point.x) > 1e-7) ||
              (std::fabs(p.y - last_point.y) > 1e-7) ||
              (std::fabs(p.z - last_point.z) > 1e-7);
        }

        if (is_new)
        {
          pl_full[i] = p;
          pl_buff[static_cast<int>(line)].push_back(p);
    ...
    for (int j = 0; j < N_SCANS; j++)
    {
      ...
      int line_plsize = static_cast<int>(pl.size());

      vector<orgtype> &types = typess[j];

      types.clear();
      types.resize(line_plsize);

      int last_idx = line_plsize - 1;
      ...
      types[last_idx].range =
          pl[last_idx].x * pl[last_idx].x +
          pl[last_idx].y * pl[last_idx].y;

      give_feature(pl, types);

      // 如果 give_feature() 内部没有往 pl_surf 填点，可以临时打开：
      // pl_surf += pl;
    }
  ...
  else
  {
    ...
    float last_curvature = 0.0f;
    bool has_last_curvature = false;
         ...
         ++i, ++iter_x, ++iter_y, ++iter_z, ++iter_i, ++iter_tag, ++iter_line, ++iter_t)
    {
      ...
      if (line < N_SCANS) 
      {
        ...
        if (!has_last_curvature)
        {
          p.curvature =
              std::fabs(raw_curvature) < 1.0f
                  ? raw_curvature
                  : 0.0f;

          last_curvature = p.curvature;
          has_last_curvature = true;
        }
        else
        {
          p.curvature =
              std::fabs(raw_curvature - last_curvature) < 1.0f
                  ? raw_curvature
                  : last_curvature + 0.004166667f;
