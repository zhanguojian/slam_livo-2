...
void loadVoxelConfig(std::shared_ptr<rclcpp::Node> node, VoxelMapConfig &voxel_config)
{
  ...
  // For vector<int>, handle the parameter as a vector
  // voxel_config.layer_init_num_ = node->declare_parameter<std::vector<int>>("lio.layer_init_num", std::vector<int>{5, 5, 5, 5, 5});
  // voxel_config.layer_init_num_ = node->declare_parameter<std::vector<int>>(
  //   "lio.layer_init_num", std::vector<int>{5, 5, 5, 5, 5});

  voxel_config.layer_init_num_ = std::vector<int>{5, 5, 5, 5, 5};
...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat)
{
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++)
  {
    ...
    if (!EKF_stop_flg && (rematch_num >= 2 || (iterCount == config_setting_.max_iterations_ - 1)))
    {
      ...
      tf_quat.setRPY(euler_cur(0), euler_cur(1), euler_cur(2)); // roll, pitch, yaw
      geoQuat_ = tf2::toMsg(tf_quat);
...
void VoxelMapManager::pubVoxelMap()
{
  ...
  for (size_t i = 0; i < pub_plane_list.size(); i++) {
    ...
    double alpha = pub_plane_list[i].is_plane_ ? use_alpha : 0;
